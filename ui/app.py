...
if st.session_state.results is not None:
    ...
    st.markdown(f"**Screening Results** &middot; <span style='color:{c['text3']}'>{len(results)} candidates</span>", unsafe_allow_html=True)
    ...
    if universal_gaps: 
        st.warning(f"These skills are missing from every candidate: {', '.join(universal_gaps)}. Consider whether they're truly required.")
    ...
    with tab3:
        ...
        csv_string = "\n".join(csv_lines)
        st.download_button("Download results as CSV", data=csv_string.encode('utf-8'),
